...
and then plot the resulting tree! 

```
obj <- contMap(tree,x,plot=T, ftype="off")
obj <- contMap(tree,y,plot=T, ftype="off")
```


We need to take the phylogeny into account … 
our trait data were simulated without an evolutionary relationship between x & y. 
The relationship that we retrieved iwith OLS is a type I error driven by the phylogenetic structure of our data. 
