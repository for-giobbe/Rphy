...
Let's import, align and infer an evolutionary model for two genes:
...
The SH (Shimodaira-Hasegawa) test is a resampling method that uses site likelihood scores 
...
```
tr1 <- pmlPart(bf ~ nni, object = list(pml_cox1, pml_nad2), optNni = TRUE)
```


... and we can see that inside the ```pml``` object there are two trees:


```
plot.phylo(tr1$fits[[1]]$tree)
plot.phylo(tr1$fits[[2]]$tree)
```
...
```
tr2 <- pmlPart(nni ~ inv, object = list(pml_cox1, pml_nad2), optNni = TRUE)
```


... and we can see that inside the ```pml``` object there is a single tree:


```
plot.phylo(tr2$fits[[1]]$tree)
plot.phylo(tr2$fits[[2]]$tree)
```
