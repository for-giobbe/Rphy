...
Let's "simulate" and align some sequences to instantly get a grasp of what it's all about:

```
nt_seq <- c("ATATCGGTA", "ACTATATATGCTG", "AGTGACAGGGTT", "TGTGCTCTACTGA")
...
```
aa_seq =  c("ARNRRTDCDCLTQCCTQTRRBWWSRDC", "ARRRTDCRRTDCCDCNTRRBWWRNRRT", "ARDCMDGARNDCLTQPDGPDCQK", "TQTRRRRTDCDCGPSWBWWBWTW")
...
There is not a single way of aligning and this tutorial we will use the most popular tools, including:
...
- the ```.phylip``` is the alignment file in the (phylip)[https://en.wikipedia.org/wiki/PHYLIP] format.
...
- [Gblocks](http://phylogeny.lirmm.fr/phylo_cgi/one_task.cgi?task_type=gblocks)
